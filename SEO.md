...
| 2026-08-07 | **Homepage title and meta description rewritten**, and separated from the visible headline for the first time. A headline is read in one glance; a title tag has to win a click from a results page. Both now name the two things the old pair omitted: productivity, and the fact that the site is bilingual |
| 2026-08-07 | **Homepage rebuilt: eleven near-identical tool grids collapsed into one tabbed section.** All three panels are pre-rendered and toggled with `hidden`, so every card is crawlable on first paint. No tool page lost coverage — `/ai-tools` and the seven category landing pages already did that job, and the categories row still links all of them |
| 2026-08-07 | **Image tools, tutorials and prompts added to the homepage**, each with a "view all" link. Three sections that were indexable but had no inbound link from the site's strongest page |
| 2026-08-07 | **The 18 PDF and image tool pages added to the site search index.** They were the only pages the site builds itself and the only ones search could not find — a query like "ضغط" returned nothing while two compression tools sat one click away. Appended last so a query like "صور" still surfaces the AI image generators first |
| 2026-08-07 | **About and Privacy Policy rewritten in both languages**, from 4 short sections to 5 and 12 respectively, with richer introductions and unique copy written for this site rather than adapted from a template. Both now carry a self-referencing `metaTitle` distinct from the `h1`, and all counts are derived from the catalogues so the pages cannot go stale |
| 2026-08-07 | **Fixed: About and Privacy emitted `BreadcrumbList` with no visible breadcrumb.** They passed verification only because the last crumb repeated the `h1`, which broke our own rule of marking up only what is on the page. Contact, the prompt pages and `/pdf-tools` still owe the same fix |
| 2026-08-07 | Post-build verification re-run after the homepage and prose-page rewrites: **176 pages, 3,129 assertions, 0 failures**, 0 broken internal links, and heading hierarchy verified with no skipped levels on all six rewritten pages |
